		...
		rlOK.setOnClickListener(new OnClickListener() {			
			...
			public void onClick(View v) {
				if(!selectedBlockNumber.equals("Select One")){
					...
					alertDialog.dismiss();
					writeMC1kValue(etNFCValue.getText().toString(), _mifareClassic1k.Header , Integer.parseInt(selectedBlockNumber));
				}
			}
		});
		alertDialog.show();
		...
		for (int i = 0; i < techList.length; i++) {
			if (techList[i].equals(MifareClassic.class.getName())) {
				...
				case MifareClassic.TYPE_CLASSIC:
					...
					boolean result = NFCHammer.writeOnClassic1K(this, tag, etNFCValue.getText().toString(), ""+HeaderNmber, ""+selectedBlockNumber);
					if(result){
